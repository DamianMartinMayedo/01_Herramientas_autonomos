-- ============================================
-- MIGRACIÓN 012: NDAs — numeración consecutiva
-- Ejecutar en: Supabase Dashboard → SQL Editor
-- ============================================

BEGIN;

-- 1) Añadir columna numero a ndas (nullable para compatibilidad con existentes)
ALTER TABLE public.ndas
  ADD COLUMN IF NOT EXISTS numero TEXT;

-- 2) Ampliar la tabla de secuencias para incluir 'nda'
ALTER TABLE public.document_sequences
  DROP CONSTRAINT IF EXISTS document_sequences_tipo_check;

ALTER TABLE public.document_sequences
  ADD CONSTRAINT document_sequences_tipo_check
  CHECK (tipo IN ('factura', 'presupuesto', 'albaran', 'contrato', 'rectificativa', 'nda'));

-- 3) Actualizar la función next_document_number para aceptar 'nda'
...
DECLARE
  ...
  v_seq INT;
BEGIN
  IF p_tipo NOT IN ('factura', 'presupuesto', 'albaran', 'contrato', 'rectificativa', 'nda') THEN
...
-- 4) Seed de secuencias a partir de NDAs existentes
INSERT INTO public.document_sequences (user_id, tipo, anio, last_value)
SELECT user_id, 'nda'::TEXT, split_part(numero, '-', 2)::INT AS anio, MAX(split_part(numero, '-', 3)::INT) AS last_value
FROM public.ndas
WHERE numero ~ '^NDA-[0-9]{4}-[0-9]+$'
